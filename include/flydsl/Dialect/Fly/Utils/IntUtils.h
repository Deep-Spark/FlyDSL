...
inline int32_t divisibilityApplySwizzleMod(int32_t lhs, SwizzleModAttr swizzle) {
  // The lowest `base` bits are never touched by the modular add, so the
  // divisibility argument mirrors the XOR swizzle case.
  return std::gcd(lhs, 1 << swizzle.getBase());
}
