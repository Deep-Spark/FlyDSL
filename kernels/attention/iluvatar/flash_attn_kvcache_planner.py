...
class _SimtDecodeLaunchPlan:
    ...
    __slots__ = ("compiled", "group_max", "group_sum", "partial_out", "stream")

    def __init__(self, entry, current_stream):
        self.compiled, self.group_max, self.group_sum, self.partial_out = entry
...
def _cached_split_bufs(
    ...
    key = ("split", device, stream_ptr, batch_size, seqlen_q, num_heads, num_splits, head_dim)
    cached = _PLACEHOLDER_CACHE.get(key)
    if cached is None:
...
def plan_decode_launch(
    *,
    ...
    # The D256/GQA4 SIMT implementation is intrinsically split: each KV head
    # needs at least two sibling CTAs.  Honour an explicit split=1 request by
    # selecting the scalar backend instead of constructing an invalid launch.
    if use_simt_decode and head_dim == 256 and num_heads == num_kv_heads * 4 and num_splits == 1:
        use_simt_decode = False
