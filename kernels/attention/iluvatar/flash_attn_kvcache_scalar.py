...
def _kv_elem_offset(
    ...
    *,
    ...
    outer_wide = fx.Int64(outer)
    head_wide = fx.Int64(head)
    token_wide = fx.Int64(token)
    d_wide = fx.Int64(d)
    if cache_strides is not None:
        ...
        return (
            outer_wide * fx.Int64(block_stride)
            + head_wide * fx.Int64(head_stride)
            + token_wide * fx.Int64(token_stride)
            + d_wide
        )
    if upstream_cache_layout:
        return (
            (outer_wide * fx.Int64(seq_cap) + token_wide)
            * fx.Int64(num_kv_heads)
            + head_wide
        ) * fx.Int64(head_dim) + d_wide
    return (
        (outer_wide * fx.Int64(num_kv_heads) + head_wide)
        * fx.Int64(seq_cap)
        + token_wide
    ) * fx.Int64(head_dim) + d_wide
