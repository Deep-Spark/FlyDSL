...
"""Double-buffered MMA decode used by the HND paged production fast path.

Scheduling (this file only)
---------------------------
* **Double-buffered K/V** SMEM ``[V0][V1][K0][K1]`` so tile ``t+1`` prefetches
  into the idle buffer while tile ``t`` computes.
* **Bounded G2S waits** via ``sl_waitmem(g2s=...)`` instead of a full barrier
  drain; the final iteration issues a dummy K prefetch so the wait immediate
  stays constant for ixcc.
* Softmax / PV numerics are shared with the baseline builder through
  ``flash_attn_decode_mma_common``.

Grid / tiling
-------------
``grid=(num_splits, batch, num_kv_heads)`` -- one CTA owns one
``(split, batch, kv_head)`` triple (same as baseline MMA). ``BM=16`` GQA rows,
tunable ``BN`` keys per tile. Interior KV tiles skip length masking; the
boundary tile still masks ``kpos >= seqlen_k_b`` (plus always-on GQA pad).

Production host routing requires HND paged cache, ``page_block_size == 16``,
bf16, ``HEAD_DIM == 128``. Dense / NHD / short contexts stay on baseline MMA,
SIMT, or scalar.
"""
...
from .flash_attn_decode_mma_common import (
    BK,
    BM,
    BN,
    NUM_WARPS,
    WARP_M,
    _sme_view_dyn,
    apply_decode_online_softmax,
    apply_decode_pv_mmad,
    bound_decode_tile_end,
    clamp_decode_group_start,
    pipelined_mma_decode_smem_bytes,
    validate_mma_decode_config,
)
...
def build_pipelined_mma_decode_attention_kernel(
    *,
    ...
    """Build the bf16 HEAD_DIM=128 double-buffered MMA decode kernel.

    Softmax/PV match the baseline builder; this path only changes K/V
    streaming. Production assembly further requires HND paged
    ``page_block_size == 16`` (enforced by ``flash_attn_kvcache_kernels``).

    When ``num_splits > 1`` each CTA covers a KV slice and writes split
    partials for the reduce kernel; otherwise it writes normalised ``Out``.

    Returns ``(kernel, threads, smem_bytes, grid)``.
    """
    BN = block_n
    repeat = validate_mma_decode_config(
        head_dim=head_dim,
        num_heads=num_heads,
    ...
    # A paged cache's physical block IDs are independent of the logical request
    # extent, so the logical-size heuristic is only valid for dense caches.
    use_i64_kv_offset = paged or cache_bytes > (1 << 31)
    ...
    k_stage_elems = BN * BK
    # Double-buffered (ping-pong) smem: [V0][V1][K0][K1].
    NUM_BUF = 2
    k_smem_offset = NUM_BUF * v_smem_elems
    smem_bytes = pipelined_mma_decode_smem_bytes(block_n=BN)
    ...
    def mma_decode_pipelined_kernel(
        ...
        # Per-sequence visible KV length (same role as varlen's ``seqlen_k_b``).
        seqlen_k_b = _load_i32(fx.get_iter(CacheSeqLens), b)
        tile_end = bound_decode_tile_end(
        ...
        q_elem_base = (
            fx.Int64(b) * fx.Int64(num_heads)
            + fx.Int64(hkv) * fx.Int64(repeat)
        ) * fx.Int64(HEAD_DIM)
        ...
        def issue_k_stage(kv_tile, k_base):
            for ni in fx.range_constexpr(k_atoms_m):
                ...
                pos_group = clamp_decode_group_start(pos_group=pos_group, seqlen_k=seqlen_k_b)
        ...
        score_lane_col = lane_id % fx.Int32(16)
        score_lane_row = lane_id // fx.Int32(16)

        # Nested body keeps thr_mma / thr_copy_A method calls out of the
        # auto-for AST, so only m_running / l_running become iter_args.
        def process_tile(kv_tile, m_running, l_running, masked):
            ...
            # K was issued before V.  Drain only the older K SME requests; keep
            # V in flight while QK runs.  Bounded g2s wait avoids the full
            # g2scnt(0) drain that a workgroup barrier inserts.
            ixdl.sl_waitmem(g2s=k_atoms_total)
            ...
            m_running, l_running = apply_decode_online_softmax(
                kv_tile=kv_tile,
                m_running=m_running,
                l_running=l_running,
                accs=accs,
                ...
                tile_start=tile_start,
                c_repeat=c_repeat,
                score_lane_col=score_lane_col,
                score_lane_row=score_lane_row,
                c_neg_inf=c_neg_inf,
                c_zero=c_zero,
                c_scale_log2e=c_scale_log2e,
                block_n=BN,
            ...
            apply_decode_pv_mmad(
                accs=accs,
                pv_accs=pv_accs,
                q_reg_frags=q_reg_frags,
                thr_mma=thr_mma,
                thr_copy_A=thr_copy_A,
                copy_atom_s2r=copy_atom_s2r,
                mma_atom=mma_atom,
                smem_ptr=smem_ptr,
                tile_atom_A=tile_atom_A,
                v_buf_base=v_base_cur,
                k_steps_pv=k_steps_pv,
                d_atoms=d_atoms,
                c16_p=_c16_p,
                cmask_p=_cmask_p,
            )
            ...
            return m_running, l_running

        kfull_count = seqlen_k_b // fx.Int32(BN)
        interior_end = (kfull_count < tile_end).select(kfull_count, tile_end)
        interior_end = (interior_end > tile_start).select(interior_end, tile_start)
        for kv_tile in range(tile_start, interior_end, fx.Int32(1)):
            m_running, l_running = process_tile(kv_tile, m_running, l_running, False)
        for kv_tile in range(interior_end, tile_end, fx.Int32(1)):
            m_running, l_running = process_tile(kv_tile, m_running, l_running, True)
        ...
        if fx.const_expr(split_mode):
            ...
            if lane_row == fx.Int32(0):
                if head_valid:
                    ...
                    # Subscript stores under dynamic control flow are rewritten as
                    # SSA writes; use memref_store for in-place tensor updates.
                    fx.memref_store(m_nat, GroupMax, (b, fx.Int32(0), head_q, split))
                    fx.memref_store(fx.Float32(l_running), GroupSum, (b, fx.Int32(0), head_q, split))
