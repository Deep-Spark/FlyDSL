...
The KV-streaming loop keeps the global->shared SME DMA busy:
...
* **Bounded G2S waits in the loop.** Workgroup barriers typically lower to
...
def build_pipelined_mma_decode_attention_kernel(
    *,
    ...
    # consumer, so the wait at s2r only blocks on the current buffer and the
    # prefetch stays in flight.  Pair with a halved block_n (e.g. BN=32) so
    # per-CTA smem matches the single-buffer baseline at BN=64.
    NUM_BUF = 2
    ...
    def mma_decode_kernel(
        ...
        cache_len = _load_i32(fx.get_iter(CacheSeqLens), b)
        ...
        for kv_tile, state in range(tile_start, tile_end, fx.Int32(1), init=loop_results):
            ...
            for ki in fx.range_constexpr(k_steps_pv):
                ...
                for nt in fx.range_constexpr(d_atoms):
                    ...
                    brick_off = v_base_cur + fx.Int32((slb_tcu_idx // 2) * BRICK_ELEMS)
                    d_sub = slb_tcu_idx % 2
