...
Ports ixinfer ``FlashDecoding_GQA::getGroupLengthAndKWarps`` (non-heuristic,
``seqs == 1``) from ``ixinfer_page_attention_get_groups_num.h``.
"""
...
def _calculate_max_splits(
    num_n_blocks: int, mr_max_blocks: int, batch_nheads_mblocks: int
) -> int:
    """Port of ixinfer ``CalculateMaxSplits``."""
    ...
    for num_splits in range(1, max_splits + 1):
        if not _is_split_eligible(num_n_blocks, num_splits):
            ...
            continue
        n_waves = batch_nheads_mblocks * num_splits / mr_max_blocks
...
def _compute_mma_decode_num_splits(
    *,
    ...
    q_warps = (
        1 if share_heads == 1 else (share_heads + q_warp_heads - 1) // q_warp_heads
    )
    ...
    least_task_size = 32
    # ixinfer can profitably use more groups because its dispatch subsequently
...
def compute_v5_decode_config(
    *,
    ...
    """Return ixinfer V5 ``(groups, fix_length, k_warps)``.
    ...
    groups = (
        1
        if base_blocks >= 0.8 * mr_max_blocks
        else _calculate_max_splits(num_n_blocks, mr_max_blocks, base_blocks)
    )
    groups = max(1, min(128, groups, (max_seqlen_k + 31) // 32))
    # The ixinfer occupancy model selects 114 groups for B1/Hq16/Hkv2/Sk32K,
    # but FlyDSL's separate group-reduce makes that slower than filling the
    # device once with 128 one-wave CTAs (64 groups * 2 KV heads).
    groups = min(groups, max(1, (_V5_TARGET_CTAS + base_blocks - 1) // base_blocks))
    fix_length = (
        (max_seqlen_k + groups - 1) // groups + _V5_CHUNK - 1
    ) // _V5_CHUNK * _V5_CHUNK
