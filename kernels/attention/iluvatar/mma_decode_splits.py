...
def compute_qwen_simt_decode_config(
    *,
    ...
    if (
        ...
        # Qwen3.5 uses 528-token physical pages, while ixinfer schedules D256
        # GQA4 in 16-token chunks. Each CTA owns two query heads; two sibling
        # CTAs cover the four heads sharing one KV head.
        logical_pages = (max_seqlen_k + 15) // 16
        # D256 needs more than two CTAs to fill the device even at 128 tokens,
        # while 32-way splitting loses to the extra workspace/reduce traffic at
        # 1K. Measured B1/H16/Hkv4 optima are 8/16/16 groups at 128/512/1024.
        if logical_pages <= 8:
    ...
    # Short contexts keep the CTA-local K-warp tree. For long contexts, match
    # ixinfer's single-sequence, non-heuristic FlashDecoding_GQA policy: split
    # the K dimension into 32-token tasks and launch one K warp per split CTA.
    # In particular, do not inflate the split count to a fixed CTA target or
    # require eight pages per split. Both choices force a 16-warp CTA just past
    # 512 tokens, where its barrier/LDS merge cost creates a latency cliff.
