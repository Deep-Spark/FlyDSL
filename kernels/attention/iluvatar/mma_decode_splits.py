    ...
    *,
    ...
    if (
        ...
        if logical_pages <= 8:
            ...
            groups = 32
        # Batch and KV-head dimensions already provide independent CTAs. Cap
        # per-sequence splits once they fill the device, avoiding thousands of
        # tiny CTAs plus proportional partial-output/reduce traffic at large B.
        base_ctas = batch_size * seqlen_q * num_kv_heads
        occupancy_groups = (_SIMT_D256_TARGET_CTAS + base_ctas - 1) // base_ctas
        groups = min(groups, occupancy_groups)
        # Keep two groups in auto mode. A direct single-group kernel is
        # available explicitly; occupancy already caps groups at large B, and
        # switching to 1 in auto would still be a distinct compiled split count.
