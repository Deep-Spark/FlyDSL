...
"""Public ``flash_attn_with_kvcache`` entry for Iluvatar.

Tensor contract (MR / upstream layouts)
---------------------------------------
...
* ``cache_seqlens`` follows upstream when k/v are separate (old lengths) and MR
  when packed (visible lengths after the packed K/V tokens are present).

Dispatch
--------
1. Validate / normalise args, optional host rotary + paged cache update.
2. ``plan_decode_launch`` picks SIMT / pipelined-MMA / baseline-MMA / scalar
   (and split count / context bucket).
3. ``build_flash_attn_with_kvcache_module`` assembles the matching kernels;
   paged D=128 prefills may instead use the varlen MMA path.
"""
...
_SIMT_BATCH_DYNAMIC_ARG_INDICES = (0, 3, 4, 5, 6, 7, 8)


def _simt_compile_signature(
    q_work,
    k_cache_view,
    v_cache_view,
    cache_seqlens,
    ...
    """Static tensor signature with batch extents stripped from leading dims."""
    return (
        q_work.dtype,
        tuple(q_work.shape[1:]),
        tuple(q_work.stride()),
...
def _mark_simt_batch_dynamic(compile_args):
    marked = list(compile_args)
    for idx in _SIMT_BATCH_DYNAMIC_ARG_INDICES:
        arg = marked[idx]
        if hasattr(arg, "mark_shape_dynamic"):
            marked[idx] = arg.mark_shape_dynamic(0)
    return tuple(marked)
...
def flash_attn_with_kvcache(
    ...
    # Reject unsupported dtypes before any cache update can be issued.
    _dtype_name(q.dtype)
    ...
    if cache_batch_idx is not None:
        ...
        if isinstance(cache_seqlens, torch.Tensor):
            ...
            if cache_seqlens.shape == (batch_size,):
                # Upstream's documented contract: lengths are already in
                # query-batch order even when cache rows are remapped.
                pass
            elif cache_seqlens.shape == (k_cache.shape[0],):
                # Also accept the useful cache-batch form and gather it.
                cache_seqlens = cache_seqlens.index_select(0, cache_row_indices)
            else:
                raise ValueError("cache_seqlens must be a [B] or [cache_batch] tensor")
    ...
    if head_dim <= 0 or head_dim > 256:
        raise ValueError("head dimension must be in [1, 256]")
    if num_heads <= 0:
    ...
    if paged:
        ...
        if page_block_size % 16 != 0:
            raise ValueError("paged cache page size must be divisible by 16")
        ...
        if tuple(k_cache_view.stride()) != tuple(v_cache_view.stride()):
            raise ValueError("paged k_cache and v_cache must have matching strides")
        ...
        if not k_cache_view.is_contiguous() or not v_cache_view.is_contiguous():
            raise ValueError("dense k_cache and v_cache must use canonical contiguous strides")
        page_block_size = 16
    ...
    host_visible_cache_len = None
    if host_cache_len is not None:
        host_visible_cache_len = host_cache_len if is_qkv_packed else host_cache_len + cache_len_delta
    strict_visible_max = None
    if strict_checks and host_cache_len is None:
        strict_visible_max = max_cache_len if is_qkv_packed else max_cache_len + cache_len_delta
    if cache_len_delta:
    ...
    if max_context_len is None:
        ...
        if max_context_len <= 0:
            raise ValueError("max_context_len must be a positive integer when provided")
        if host_visible_cache_len is not None and max_context_len < host_visible_cache_len:
            raise ValueError("max_context_len must cover the host-known visible cache length")
        if strict_visible_max is not None and max_context_len < strict_visible_max:
            raise ValueError("max_context_len must cover the actual visible cache lengths")
    ...
    if has_rotary:
        ...
        if rotary_cols not in (head_dim, head_dim // 2):
            raise ValueError("rotary_cos/sin last dimension must be head_dim or head_dim // 2")
        if host_visible_cache_len is not None:
            visible_bound = host_visible_cache_len
        elif strict_visible_max is not None:
            visible_bound = strict_visible_max
        elif max_context_len is not None:
            visible_bound = min(max_context_len, max_seqlen_k)
        else:
            # Non-strict tensor metadata cannot be read without synchronizing.
            # Requiring capacity-sized tables keeps the hot path asynchronous.
            visible_bound = max_seqlen_k
        if update_cache:
            required_rotary_rows = visible_bound
        else:
            query_span = seqlen_q if causal or window_size != (-1, -1) else 1
            required_rotary_rows = visible_bound + query_span
        if rotary_cos.shape[0] < required_rotary_rows:
            raise ValueError(
                "rotary_cos/sin have insufficient rows for reachable positions: "
                f"need {required_rotary_rows}, got {rotary_cos.shape[0]}"
            )
        if not update_cache:
    ...
    if update_cache and paged:
        ...
        # Resolve every destination before writing the first cache element.
        # Device-synchronizing metadata validation remains opt-in; enabling it
        # on the decode hot path would dominate kernel latency.
        logical_blocks = kv_positions // page_block_size
        if strict_checks:
            if bool((kv_positions < 0).any().item()):
                raise ValueError("paged cache update positions must be non-negative")
            if bool((logical_blocks >= block_table.shape[1]).any().item()):
                raise ValueError("paged cache update exceeds block_table capacity")
        physical_blocks_for_update = block_table.gather(1, logical_blocks.to(torch.long))
        if strict_checks:
            physical_min, physical_max = physical_blocks_for_update.aminmax()
            if physical_min.item() < 0 or physical_max.item() >= k_cache_view.shape[0]:
                raise ValueError("paged cache update block_table entries must index physical cache blocks")
        if causal or window_size != (-1, -1):
        ...
        physical_blocks = physical_blocks_for_update.to(torch.long)
        block_offsets = (kv_positions % page_block_size).to(torch.long)
        if upstream_cache_layout:
            k_cache_view[physical_blocks, block_offsets] = k_part
            v_cache_view[physical_blocks, block_offsets] = v_part
        else:
            k_cache_view[physical_blocks, :, block_offsets, :] = k_part
            v_cache_view[physical_blocks, :, block_offsets, :] = v_part
        q = q_part.contiguous()
    ...
    if effective_num_splits > 1:
        ...
        group_max, group_sum, partial_out = _cached_split1_bufs(q.device, current_stream.cuda_stream)
    launcher = build_flash_attn_with_kvcache_module(
        # SIMT decode reads B from QWork.shape[0] so one compiled launcher
        # covers mixed vLLM batches. MMA/scalar grids still close over B.
        batch_size=0 if (use_simt_decode and not update_cache) else batch_size,
        seqlen_q=seqlen_q,
        num_heads=num_heads,
        num_kv_heads=num_kv_heads,
        head_dim=head_dim,
        # SIMT split extents come from runtime lengths and compile against
        # capacity. Pipelined MMA bakes tile partitions into the kernel, so it
        # must compile against the planner's active-context bucket.
        max_seqlen_k=(
            planning_max_seqlen_k
    ...
    if compiled is None:
        if static_simt_decode:
            compile_args = _mark_simt_batch_dynamic(
                tuple(
    ...
    if fast_probe is not None and use_simt_decode and not update_cache:
        store_simt_decode_entry(
            fast_probe[0],
            (compiled, effective_num_splits, seqlen_q, num_heads, head_dim),
        )
