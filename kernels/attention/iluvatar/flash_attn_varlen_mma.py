...
Only bf16, ``head_dim == 128`` is handled (the tensor-core inner math is the
proven HEAD_DIM=128 sequence from ``iluvatar_mr_flash_attn.py``); everything
else stays on other paths.
...
from flydsl._mlir.dialects.arith import (
    TruncFOp, BitcastOp, ExtUIOp, ShLIOp, OrIOp, AndIOp, ShRUIOp,
)
from flydsl._mlir.dialects import llvm as _llvm, math as _math_dialect
...
def flash_attn_varlen_smem_bytes() -> int:
    v_smem_elems = HEAD_DIM * BN
    k_stage_elems = BN * BK
    return (v_smem_elems + k_stage_elems) * 2


def select_num_warps(max_seqlen_q: int) -> int:
...
def varlen_grid(causal_balance: str, num_heads: int, batch: int,
                max_q_tiles: int):
    """Launch grid for a given causal-balance scheme.

    * ``head_fastest``: (num_heads, batch, max_q_tiles) -- head varies fastest
      so each dispatch wave holds one q-tile across heads (equal causal work).
    * ``head_parity``: (max_q_tiles, batch, num_heads) -- q-block varies fastest
      (ixinfer style); even heads reverse the q-block order so adjacent head
...
def build_flash_attn_varlen_kernel(
    *,
    ...
    """Build the bf16 HEAD_DIM=128 varlen prefill attention kernel.
    ...
    """
    assert head_dim == HEAD_DIM, "varlen kernel only supports head_dim == 128"
    assert num_heads % num_kv_heads == 0
    assert page_block_size % SME_ROWS == 0, "page_block_size must be a multiple of 16"
    assert causal_balance in ("head_fastest", "head_parity"), (
        f"unknown causal_balance {causal_balance!r}")
    ...
    assert cta_atoms_k % k_per_warp == 0, (
        "each 16-row KV group's bricks must be split across a whole number of warps")
    ...
    assert BM * HEAD_DIM <= total_smem_elems, (
        "Q tile does not fit in shared memory; reduce num_warps")
    ...
    def flash_attn_varlen_kernel(
        ...
        if fx.const_expr(causal_balance == "head_parity"):
            # ixinfer-style: grid (max_q_tiles, batch, num_heads), q-block varies
            ...
            # wave holds CTAs with the *same* q-tile -- identical causal work (a
            # causal q-tile ``t`` streams KV ``[0, t]``) -- balancing the load
            # with no per-wave straggler.  Under GQA adjacent heads share a KV
            # head, so a wave also reuses the same K/V in L2.
        ...
        if fx.const_expr(paged):
            lane_id = fx.Int32(
                _llvm.call_intrinsic(T.i32, "llvm.bi.lane.id", [], [], [])
            )
        else:
        ...
        def _load_cu_q(idx):
            div = fx.logical_divide(CuSeqlensQ, fx.make_layout(1, 1))
            return _load_i32(fx.slice(div, (None, idx)))

        def _load_seqused_k():
            div = fx.logical_divide(SequsedK, fx.make_layout(1, 1))
            return _load_i32(fx.slice(div, (None, b)))

        def _load_block(logical_block):
            table_row = fx.slice(BlockTable, (b, None))
            logical_block = fx.Int32(
                _llvm.call_intrinsic(
                    T.i32, "llvm.bi.readfirstlane", [_to_raw(logical_block)], [], []
                )
            )
            ...
            table_base = fx.inttoptr(
                fx.PointerType.get(T.i32, fx.AddressSpace.Generic), table_base_addr
            )
            loaded = fx.ptr_load(
                fx.add_offset(table_base, fx.make_int_tuple(logical_block)), T.i32
            )
            return fx.Int32(
                _llvm.call_intrinsic(T.i32, "llvm.bi.readfirstlane", [_to_raw(loaded)], [], [])
            )
        ...
        dummy_ptr = fx.recast_iter(
            fx.PointerType.get(fx.Float32.ir_type, fx.AddressSpace.Shared), smem_ptr)
        ...
        def _make_k_atoms():
            ...
            for jm in fx.range_constexpr(WARP_ATOMS_M):
                ...
                for ki in fx.range_constexpr(cta_atoms_k):
                    off = fx.Int32(
                        k_smem_offset + (jm * cta_atoms_k + ki) * BRICK_ELEMS)
        ...
        q_sme_tile = _make_bf16_gmem_tile(
            Q, q_elem_base, (BM, HEAD_DIM), (q_row_stride, 1))
        ...
        out_i32_ptr = fx.recast_iter(
            fx.PointerType.get(T.i32, fx.AddressSpace.Global), fx.get_iter(O))
        ...
        kv_stride = HEAD_DIM if (paged and not upstream_cache_layout) \
        ...
        k_smem_i32 = fx.Tensor(fx.make_view(
            fx.recast_iter(
        ...
        dummy_bf16_ptr = fx.recast_iter(
            fx.PointerType.get(fx.BFloat16.ir_type, fx.AddressSpace.Shared), smem_ptr)
        ...
        if fx.const_expr(causal):
            ...
            cs_count = (cs_limit >= fx.Int32(0)).select(
                cs_limit // c_BN + fx.Int32(1), fx.Int32(0))
        ...
        for em_idx in fx.range_constexpr(4):
            v_col_em.append(
                v_sload_base + ((v_sload_key ^ fx.Int32(em_idx)) * fx.Int32(4)))

        def process_tile(kv_tile, m_running, l_running, kv_state, masked, kv_v_state=None):
            if fx.const_expr(not paged):
                # Carry the SME fat pointers themselves, as ixinfer's
                ...
                issue_v_from_tensor(fx.Tensor(fx.make_view(
                    v_ptr, fx.make_layout((SME_ROWS, BK), (kv_stride, 1)))))
            else:
            ...
            if fx.const_expr(paged):
                ...
                pos_next = ((fx.Int32(kv_tile) + fx.Int32(1)) * c_BN
                            + ni_warp * fx.Int32(SME_ROWS))
            ...
            def _qk_softmax(m_cur, l_cur):
                ...
                def _score_valid(jm, v):
                    kpos = (fx.Int32(kv_tile) * c_BN
                            + fx.Int32(jm * ATOM_N + v * 4) + score_lane_row)
                ...
                for jm in fx.range_constexpr(WARP_ATOMS_M):
                    ...
                    for v in fx.range_constexpr(FRAG_ELEMS):
                        fma_val = _math_dialect.fma(
                            ...
                            _to_raw(c_scale_log2e), neg_m_new)
                ...
                l_new = fx.Float32(
                    _math_dialect.fma(
                        _to_raw(l_cur), _to_raw(corr), _to_raw(local_sum)))
                return [m_new, l_new]
            ...
            def _pack_p_frags():
                ...
                for ki in fx.range_constexpr(k_steps_pv):
                    ...
                    for v in fx.range_constexpr(FRAG_ELEMS // 2):
                        a = arith.ArithValue(
                            _to_raw(p_vals[v * 2])).bitcast(T.i32)
                        bb = arith.ArithValue(
                            _to_raw(p_vals[v * 2 + 1])).bitcast(T.i32)
                        pk.append(OrIOp(
                            AndIOp(bb, _cmask_p).result,
                            ShRUIOp(a, _c16_p).result).result)
                    packed.append(vector.bitcast(
                        T.vec(FRAG_ELEMS, T.bf16),
                        vector.from_elements(
            ...
            if fx.const_expr(not paged):
                ...
                if fx.Int32(kv_tile) != end_tile - fx.Int32(1):
                    issue_k_from_tensor(fx.Tensor(fx.make_view(
                        k_ptr_next,
                        fx.make_layout((SME_ROWS, BK), (kv_stride, 1)))))
            else:
                if fx.Int32(kv_tile) != end_tile - fx.Int32(1):
        ...
        if fx.const_expr(not paged):
            ...
            k_tensor0 = _make_bf16_gmem_tile(
                KCache, kv_base0, (SME_ROWS, BK), (kv_stride, 1))
            v_tensor0 = _make_bf16_gmem_tile(
                VCache, kv_base0, (SME_ROWS, BK), (kv_stride, 1))
        ...
        if fx.const_expr(not paged):
            ...
            for kv_tile in range(0, interior_end, 1):
                m_running, l_running, k_ptr, v_ptr = process_tile(
                    kv_tile, m_running, l_running,
                    k_ptr, masked=False, kv_v_state=v_ptr)
            ...
            for kv_tile in range(interior_end, end_tile, 1):
                m_running, l_running, k_ptr, v_ptr = process_tile(
                    kv_tile, m_running, l_running,
                    k_ptr, masked=True, kv_v_state=v_ptr)
        else:
            kv_base = _to_raw(kv_base0)
            for kv_tile in range(0, interior_end, 1):
                m_running, l_running, kv_base = process_tile(
                    kv_tile, m_running, l_running, kv_base, masked=False)

            for kv_tile in range(interior_end, end_tile, 1):
                m_running, l_running, kv_base = process_tile(
                    kv_tile, m_running, l_running, kv_base, masked=True)
        ...
        # ---- Epilogue: normalise + TransposeCToB16 (HEAD_DIM=128) ----
