    ...
    *,
    ...
    def flash_attn_varlen_kernel(
        ...
        if fx.const_expr(paged):
            ...
            # cu_seqlens_k defines each packed sequence's storage start, while
            # seqused_k is the caller-visible prefix within that span.
            seqlen_k_b = _load_i32(SequsedK, b)
        ...
        def _kv_group_base(pos_group, clamp=True):
            ...
            if fx.const_expr(not paged):
                if fx.const_expr(clamp):
                    ...
                    last_group = (
                        (seqlen_k_b - fx.Int32(1)) // fx.Int32(SME_ROWS)
                    ) * fx.Int32(SME_ROWS)
                    pos_c = (pos_group < seqlen_k_b).select(
                ...
                gpos = k_start + pos_c
                base = (
                    fx.Int64(gpos) * fx.Int64(num_kv_heads) + fx.Int64(hkv)
                ) * fx.Int64(head_dim)
            ...
            # Empty paged rows may legally retain a -1 sentinel in slot zero
            # when strict host checks are disabled. The pipelined prologue
            # still issues one speculative DMA for a zero-iteration KV loop,
            # so redirect that descriptor to physical block zero.
            phys = (seqlen_k_b > fx.Int32(0)).select(phys, fx.Int64(0))
            base = (
                phys * fx.Int64(cache_block_stride)
                + fx.Int64(hkv) * fx.Int64(cache_head_stride)
                + fx.Int64(block_off) * fx.Int64(cache_page_stride)
            )
        ...
        def process_tile(kv_tile, m_running, l_running, kv_state, masked, kv_v_state=None):
            if fx.const_expr(not paged):
                ...
                issue_v_from_base(fx.Int64(kv_state))
        ...
        # Write base (before S<3,1,5>): addr = 2*i + 16*(tid&31) + (tid>>5).
        # Bit map (a0=LSB): [a8..a0] = [t4 t3 t2 t1 t0 | i2 i1 i0 | t5]
        #   with i=[i2|i1|i0], tid=[t5|t4|t3|t2|t1|t0] (t0=LSB).
        tc_write_lyt = fx.make_composed_layout(
            fx.static(fx.SwizzleType.get(3, 1, 5)),
            fx.make_layout((8, (2, 2, 2, 2, 2, 2)), (2, (16, 32, 64, 128, 256, 1))),
        )
        # Read base: addr = 64*i + tid → [a8..a0] = [i2 i1 i0 | t5 t4 t3 t2 t1 t0].
