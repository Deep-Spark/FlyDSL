...
def build_flash_attn_varlen_kernel(
    *,
    ...
    def flash_attn_varlen_kernel(
            ...
            def _pv_mmad(packed_p):
                ...
                for ki in fx.range_constexpr(k_steps_pv):
                    ...
                    for nt in fx.range_constexpr(d_atoms):
                        ...
                        # SME Col stores this brick logically as V[key, d].
                        # Compose it with a (d, k)->(key, d) coordinate map:
                        # key = pi(k), where pi([b3|b2|b1|b0]) =
                        # [b3|b0|b2|b1].  The permutation is now part of the
                        # source layout rather than explicit lane-address math.
        ...
        # Address maps as layouts; R→S / S→R / R→G via fx.copy
        # (see why/fa_epilogue_c_to_b16_layout.py). Pack/repack stay arithmetic.
