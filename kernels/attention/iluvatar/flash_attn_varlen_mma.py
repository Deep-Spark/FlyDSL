...
def build_flash_attn_varlen_kernel(
    *,
    ...
    def flash_attn_varlen_kernel(
        ...
        kv_stride = HEAD_DIM if (paged and not upstream_cache_layout) else (num_kv_heads * HEAD_DIM)
        ...
        def load_k_frags(sK, kk):
            ...
            for jm in fx.range_constexpr(WARP_ATOMS_M):
                ...
                frag_K.store(
                    vector.bitcast(
                        T.vec(FRAG_ELEMS, T.bf16), vector.from_elements(T.vec(FRAG_ELEMS // 2, T.i32), [k_i0, k_i1])
                    )
                )
                ######################## end ############################
