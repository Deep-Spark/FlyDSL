...
"""Double-buffered MMA implementation used by the V5 HND decode fast path.

``flash_attn_kvcache_v5.py`` constrains this builder to a 16-token HND page and
selects it automatically.  The KV-streaming loop mirrors how ixinfer's
...
* **Dual prefetch**: the *next* tile's K and V are both issued during the current
  tile's compute (ixinfer issues ``FetchKeyFromGlobal`` / ``FetchValueFromGlobal``
  one block ahead), not just K.

The production V5 configuration uses ``block_n=16`` and an occupancy-capped
group count; larger blocks remain supported only for baseline experiments.
...
* One CTA owns one ``(batch, kv_head)`` pair and streams the dense cache for that
  head through the QK and PV MMADs with online softmax.
...
def build_mma_decode_attention_kernel(
    *,
    ...
    # the current buffer -- the prefetch stays in flight (ixinfer-style overlap).
    ...
    def mma_decode_kernel(
            ...
            if fx.const_expr(use_i64_kv_offset):
                ...
                if fx.const_expr(paged):
                    ...
                    if fx.const_expr(upstream_cache_layout):
                        base = (
                            (phys * c_page_wide + block_off) * c_Hkv_wide + hkv_wide
                        ) * c_D_wide
                        stride = num_kv_heads * HEAD_DIM
                    else:
                        base = (
                            (phys * c_Hkv_wide + hkv_wide) * c_page_wide + block_off
                        ) * c_D_wide
                        stride = HEAD_DIM
                else:
                    if fx.const_expr(upstream_cache_layout):
                        base = (
                            (fx.Int64(b) * c_S_wide + pos_group_wide) * c_Hkv_wide
                            + hkv_wide
                        ) * c_D_wide
                        stride = num_kv_heads * HEAD_DIM
                    else:
                        base = (
                            (fx.Int64(b) * c_Hkv_wide + hkv_wide) * c_S_wide
                            + pos_group_wide
                        ) * c_D_wide
                        stride = HEAD_DIM
            else:
        ...
        for kv_tile, _iter_args, _loop_results in scf.for_(
            ...
            # G2S-count protocol used by ixinfer and avoids the full g2scnt(0)
