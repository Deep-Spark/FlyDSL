...
"""Baseline (single-buffer) MMA decode kernel for ``flash_attn_with_kvcache``.

Tensor-core (MMAD) decode attention specialised for the KV-cache *decode* case:
...
* One CTA owns one ``(split, batch, kv_head)`` triple
  (``grid=(num_splits, batch, num_kv_heads)``).

Grid / tiling
-------------
``BM = 16`` query rows (GQA group), ``BN`` keys per KV tile (tunable, default
128). Softmax / PV live in ``flash_attn_decode_mma_common``; this file owns the
single-buffer K→V→compute schedule. Full KV tiles skip length masking; only the
last partial tile (plus GQA pad rows) pay per-element selects -- same contract
as ``flash_attn_varlen_mma``, without causal.

Covers bf16 ``HEAD_DIM == 128`` for dense and paged HND/NHD caches, including
split-KV. Long HND paged production traffic prefers the pipelined builder.
"""
...
def build_mma_decode_attention_kernel(
    *,
    ...
    """Build the bf16 HEAD_DIM=128 single-buffer MMA decode kernel.
    ...
    k_stage_elems = BN * BK
    smem_bytes = decode_mma_smem_bytes(block_n=BN, num_buffers=1)
    ...
    def mma_decode_kernel(
        ...
        def _kv_group_base(pos_group):
            ...
            if fx.const_expr(paged):
                ...
                # Physical block IDs are not bounded by this request's logical
                # sequence extent.  Keep all paged address products in i64.
                block_off = fx.Int64(pos_group) % fx.Int64(page_block_size)
                phys = fx.Int64(_load_block(logical_block))
                phys_hkv = fx.Int64(num_kv_heads)
                phys_page = fx.Int64(page_block_size)
                phys_d = fx.Int64(HEAD_DIM)
                hkv_wide = fx.Int64(hkv)
                if fx.const_expr(upstream_cache_layout):
                    base = ((phys * phys_page + block_off) * phys_hkv + hkv_wide) * phys_d
                    stride = num_kv_heads * HEAD_DIM
                else:
                    base = ((phys * phys_hkv + hkv_wide) * phys_page + block_off) * phys_d
                    stride = HEAD_DIM
            else:
                b_wide = fx.Int64(b)
                pos_wide = fx.Int64(pos_group)
                hkv_wide = fx.Int64(hkv)
                if fx.const_expr(upstream_cache_layout):
                    base = (
                        (b_wide * fx.Int64(max_seqlen_k) + pos_wide)
                        * fx.Int64(num_kv_heads)
                        + hkv_wide
                    ) * fx.Int64(HEAD_DIM)
                    stride = num_kv_heads * HEAD_DIM
                else:
                    base = (
                        (b_wide * fx.Int64(num_kv_heads) + hkv_wide)
                        * fx.Int64(max_seqlen_k)
                        + pos_wide
                    ) * fx.Int64(HEAD_DIM)
                    stride = HEAD_DIM
            return base, stride
        ...
        # Lane→(query-row, key-subrow) mapping is invariant across KV tiles.
        ...
        # Interior tiles are fully covered by ``seqlen_k_b``; only the tail pays
        # length masking (GQA pad masking still applies on both paths).
