    ...
    *,
    ...
    if use_simt_decode and not update_cache:
        ...
        def launch_simt_decode(
            ...
            # Batch is a runtime grid extent so mixed vLLM batches share one
            # compiled launcher. num_splits / heads stay compile-time.
            bsz = QWork.shape[0]
            if hasattr(bsz, "unpack"):
                bsz = bsz.unpack()
            simt_decode_kernel(
                QWork, KCache, VCache, CacheSeqLens, BlockTable, Out, GroupMax, GroupSum, PartialOut
            ).launch(
                grid=(bsz, num_splits, num_kv_heads),
