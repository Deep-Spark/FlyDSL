...
mis-handled, matching the philosophy of ``flash_attn_with_kvcache_flydsl``.

Dispatch:
* ``max_seqlen_q == 1`` (pure decode) is routed to the proven decode kernels via
  ``flash_attn_with_kvcache_flydsl`` (ixinfer / MMA tensor-core decode).
...
def _build_prefill_launcher(
    *,
    ...
    # Grid (heads, batch, q_tiles): head fastest balances causal load per wave.
    grid = (num_heads, batch, max_q_tiles)

    @flyc.jit
    def launch(Q, K, V, CuQ, SeqK, BT, Oo, stream=fx.Stream(None)):
        kernel(Q, K, V, CuQ, SeqK, BT, Oo).launch(
            grid=grid, block=(threads, 1, 1), smem=smem, stream=stream)

    return launch


def _prefill(
    q, k, v, cu_seqlens_q, seqused_k, block_table,
    *, num_heads, num_kv_heads, head_dim, page_block_size, causal,
    upstream_cache_layout, softmax_scale, batch, max_seqlen_q, out, stream,
    paged=True, cu_seqlens_k=None,
):
    total_q = q.shape[0]
    ...
    q_pad = torch.empty((total_q + bm, num_heads, head_dim),
    ...
    if paged:
        ...
        total_k = k.shape[0]
        k_pad = torch.zeros((total_k + bm, num_kv_heads, head_dim),
                            device=k.device, dtype=k.dtype)
        k_pad[:total_k].copy_(k)
        v_pad = torch.zeros((total_k + bm, num_kv_heads, head_dim),
    ...
    out_buf = torch.empty((total_q + bm, num_heads, head_dim),
                          device=q.device, dtype=q.dtype)

    key = (num_heads, num_kv_heads, head_dim, page_block_size, bool(causal),
           bool(upstream_cache_layout), round(float(softmax_scale), 8),
           batch, max_q_tiles, num_warps, bool(paged))
    args = (q_pad.reshape(-1), k_flat, v_flat,
            cu_seqlens_q, seqused_k, bt_arg, out_buf.reshape(-1), stream)

    compiled = _PREFILL_CACHE.get(key)
    if compiled is None:
        launcher = _build_prefill_launcher(
            num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim,
            page_block_size=page_block_size, causal=causal,
            upstream_cache_layout=upstream_cache_layout,
            softmax_scale=softmax_scale, batch=batch, max_q_tiles=max_q_tiles,
            num_warps=num_warps, paged=paged,
        )
...
def _decode_dispatch(
    q, k_cache, v_cache, seqused_k, block_table,
    *, num_heads, num_kv_heads, head_dim, causal, out, batch,
    upstream_cache_layout,
):
    """Route pure-decode (max_seqlen_q == 1) to the proven decode kernels."""
    os.environ.setdefault("FLYDSL_KVCACHE_IXINFER", "1")
    from .flash_attn_kvcache import flash_attn_with_kvcache_flydsl

    # Packed [batch, num_heads, D] -> decode wrapper's [B, 1, Hq, D].
    q_dec = q.reshape(batch, 1, num_heads, head_dim).contiguous()
    res = flash_attn_with_kvcache_flydsl(
...
def flash_attn_varlen_func_flydsl(
    q,
    k,
    v,
    cu_seqlens_q,
    ...
    *,
    ...
    if paged:
        ...
        if k.ndim != 3 or v.ndim != 3:
            raise ValueError(
                "dense (non-paged) k/v must be 3D [total_k, num_kv_heads, head_dim]")
    ...
    if paged:
        if block_table.shape[0] != batch:
            raise ValueError("block_table batch must match cu_seqlens_q")
    ...
    default_scale = abs(softmax_scale - head_dim ** (-0.5)) < 1e-9
    if (paged and use_decode_kernel and max_seqlen_q == 1
            and q.shape[0] == batch and default_scale):
        return _decode_dispatch(
            q, k, v, seqused_k, block_table,
            num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim,
            causal=causal, out=out, batch=batch,
            upstream_cache_layout=upstream_cache_layout,
        )
    ...
    return _prefill(
        ...
        num_heads=num_heads, num_kv_heads=num_kv_heads, head_dim=head_dim,
        page_block_size=page_block_size, causal=causal,
        upstream_cache_layout=upstream_cache_layout, softmax_scale=softmax_scale,
        batch=batch, max_seqlen_q=max_seqlen_q, out=out, stream=stream,
        paged=paged, cu_seqlens_k=ck_i32,
    )
