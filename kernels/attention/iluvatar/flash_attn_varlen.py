...
* Either KV source:
  - *Paged* cache (``block_table`` given), layout selected by
    ...
    ``VLLM_KV_CACHE_LAYOUT=HND``) or NHD (default)
...
def flash_attn_varlen_func(
    ...
    *,
    ...
    ``k``/``v``: paged cache, layout selected by ``kv_cache_layout``:
        * ``"HND"``: ``[num_blocks, num_kv_heads, page, head_dim]``
          (vLLM ``VLLM_KV_CACHE_LAYOUT=HND``).
        * ``"NHD"`` (default): ``[num_blocks, page, num_kv_heads, head_dim]``
          (vLLM default layout).
    ...
    if paged:
        ...
        if not k.is_contiguous() or not v.is_contiguous():
            raise ValueError("dense k/v must be contiguous")
    if num_kv_heads <= 0:
        raise ValueError("number of KV heads must be positive")
    if num_heads % num_kv_heads != 0:
        raise ValueError("num_heads must be divisible by num_kv_heads")
    if page_block_size <= 0 or page_block_size % 16 != 0:
        raise ValueError("page_block_size must be a positive multiple of 16")
    ...
    elif q.shape[0] > batch * max_seqlen_q:
        raise ValueError("max_seqlen_q cannot cover total_q across the batch")
