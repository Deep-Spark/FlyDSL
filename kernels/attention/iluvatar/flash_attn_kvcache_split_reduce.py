...
def build_split_reduce_kernels(*, head_dim: int, num_splits: int, dtype_str: str):
    ...
    def split_reduce_kernel(
        ...
        if tid < head_dim:
            ...
            fx.memref_store(
                has_tokens.select(acc / denom, zero).to(elem_dtype),
                Out,
                (b, row, hq, d),
            )
    ...
    def pipelined_split_reduce_kernel(
        ...
        """reduction for the pipelined MMA split workspace."""
        row = fx.Int32(fx.block_idx.x)
        b = fx.Int32(fx.block_idx.y)
        hq = fx.Int32(fx.block_idx.z)
        tid = fx.thread_idx.x
        ...
        if lane == fx.Int32(0):
            # Subscript stores under dynamic control flow are rewritten as SSA
            # writes; use memref_store for in-place smem/tensor updates.
        ...
        if warp == fx.Int32(0):
            ...
            fx.memref_store(
                has_tokens.select(warp_out0 / warp_sum, zero).to(elem_dtype),
                Out,
                (b, row, hq, d0),
            )
            fx.memref_store(
                has_tokens.select(warp_out1 / warp_sum, zero).to(elem_dtype),
                Out,
                (b, row, hq, d1),
            )
            if fx.const_expr(head_dim > 128):
                fx.memref_store(
                    has_tokens.select(warp_out2 / warp_sum, zero).to(elem_dtype),
                    Out,
                    (b, row, hq, d2),
                )
                fx.memref_store(
                    has_tokens.select(warp_out3 / warp_sum, zero).to(elem_dtype),
                    Out,
                    (b, row, hq, d3),
                )
