...
def _prefill(
    ...
    *,
    ...
    # A single sequence whose packed row count is SME-aligned has no partial
    # 16-row brick, so the kernel can read Q directly. General varlen batches
    # retain the padded fallback because individual sequence lengths are only
    # available in device metadata here.
    if batch == 1 and total_q % SME_ROWS == 0:
        q_pad = q
    else:
...
def _get_varlen_prefill_launch_plan(
    ...
    *,
    ...
    if not q.is_cuda or batch != 1 or q.shape[0] % SME_ROWS != 0 or out is None or k.ndim != 4 or block_table.ndim != 2:
        return None
