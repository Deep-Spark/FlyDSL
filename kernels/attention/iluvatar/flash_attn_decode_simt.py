    ...
    def simt_decode_kernel(
        ...
        if fx.const_expr(split_mode):
            if x == fx.Int32(0) and y == fx.Int32(0):
                GroupMax[b, 0, qh0, split] = (l0 > zero).select(m0, reduce_neg_inf)
                GroupMax[b, 0, qh1, split] = (l1 > zero).select(m1, reduce_neg_inf)
                GroupSum[b, 0, qh0, split] = l0
                GroupSum[b, 0, qh1, split] = l1
            if x == fx.Int32(0):
                for r in range_constexpr(dot_repeats):
                    d = fx.Int32((r // 4) * 32 + (r % 4) * 8) + y * fx.Int32(2)
                    PartialOut[b, 0, qh0, split, d] = o0[2 * r]
                    PartialOut[b, 0, qh0, split, d + 1] = o0[2 * r + 1]
                    PartialOut[b, 0, qh1, split, d] = o1[2 * r]
                    PartialOut[b, 0, qh1, split, d + 1] = o1[2 * r + 1]
        else:
            ...
            if x == fx.Int32(0) and y == fx.Int32(0):
                ...
                partials[base] = (l0 > zero).select(m0, reduce_neg_inf)
                partials[base + 1] = l0
                partials[base + (2 + head_dim)] = (l1 > zero).select(m1, reduce_neg_inf)
                partials[base + (3 + head_dim)] = l1
            if x == fx.Int32(0):
                for r in range_constexpr(16):
                    d = fx.Int32((r // 4) * 32 + (r % 4) * 8) + y * fx.Int32(2)
                    partials[base + 2 + d] = o0[2 * r]
                    partials[base + 3 + d] = o0[2 * r + 1]
                    partials[base + (4 + head_dim) + d] = o1[2 * r]
                    partials[base + (5 + head_dim) + d] = o1[2 * r + 1]
            gpu.barrier()
            ...
            if k_warp == fx.Int32(0) and x == fx.Int32(0):
                if seqlen_k_b > fx.Int32(0):
                    max0, den0 = merge_stats(0)
                    max1, den1 = merge_stats(1)
                    for r in range_constexpr(dot_repeats):
                        d = fx.Int32((r // 4) * 32 + (r % 4) * 8) + y * fx.Int32(2)
                        Out[b, 0, qh0, d] = (merge_value(0, d, max0) / den0).to(fx.BFloat16)
                        Out[b, 0, qh0, d + 1] = (merge_value(0, d + 1, max0) / den0).to(fx.BFloat16)
                        Out[b, 0, qh1, d] = (merge_value(1, d, max1) / den1).to(fx.BFloat16)
                        Out[b, 0, qh1, d + 1] = (merge_value(1, d + 1, max1) / den1).to(fx.BFloat16)
                else:
                    for r in range_constexpr(dot_repeats):
                        d = fx.Int32((r // 4) * 32 + (r % 4) * 8) + y * fx.Int32(2)
                        Out[b, 0, qh0, d] = fx.BFloat16(0.0)
                        Out[b, 0, qh0, d + 1] = fx.BFloat16(0.0)
                        Out[b, 0, qh1, d] = fx.BFloat16(0.0)
                        Out[b, 0, qh1, d + 1] = fx.BFloat16(0.0)
