...
from flydsl.expr.primitive import inttoptr, ptrtoint
...
def _smex_loadn_view(smem_base, *, tile_base_elems, em_part_addend: int, shape_mn_k):
    ...
    if int(em_part_addend) == 0:
        ...
        addr = ptrtoint(brick_ptr)
        ptr_em = inttoptr(brick_ptr.type, addr + int(em_part_addend))
    mn, k = shape_mn_k
    return fx.make_view(ptr_em, fx.make_layout((mn, k), (1, mn)))
...
def cq_gemm_s2r_a_tile(
    *,
    ...
    tile_base_elems = (global_mn * fx.Int32(k_atoms // 2) + fx.Int32(mma_k // 2)) * fx.Int32(
        geom.cta_chunk_elems
    )
