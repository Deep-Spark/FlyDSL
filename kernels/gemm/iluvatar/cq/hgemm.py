...
"""CQ HGEMM bring-up: optional ``mma_tile`` over CQMma (base + FeatureLongMtx).

This is the long-mtx correctness entry, not the full pipelined CQ HGEMM
(G2S / loadn / named-barrier). It issues a single-warp ``CQMma`` with constant
A/B register fragments and stores the C fragment -- enough to validate long-mtx
ThrVal layouts + ``ixdl.mmad`` lowering. Default ``mma_tile=\"base\"`` keeps the
16x16x16 path.
"""
...
__all__ = ["compile_iluvatar_cq_hgemm"]
