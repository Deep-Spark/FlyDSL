...
def _build_igemm_kernel(
    ...
    assert warp_atoms_m >= 2 and warp_atoms_m % 2 == 0, (
...
def compile_iluvatar_mr_igemm(
    *,
    ...
    if warp_atoms_m < 2 or warp_atoms_m % 2 != 0:
        raise ValueError(
            f"warp_atoms_m={warp_atoms_m} must be even and >= 2 (half-M S2R/MMA schedule)"
        )
    ...
    stages = resolve_igemm_stages(
        stages, M, N, K, bm, bn, bk, major_pattern=major_pattern, epilogue=epilogue
    )
