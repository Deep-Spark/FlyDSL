...
def build_iluvatar_silu_and_mul_module(
    ...
    if quant_mode != "none":
        raise ValueError(f"Iluvatar b16 silu_and_mul supports only quant_mode='none', got {quant_mode!r}")
    ...
    def moe_silu_and_mul(
        ...
        is_valid = (bid < fx.Int32(num_valid_ids[0])) & (token_id < token_num) & (slot_id < k)
        ...
        if is_valid:
            ...
            route = fx.Float32(sorted_weights[bid]) if const_expr(apply_route_weight) else fx.Float32(1.0)
            ...
            for base in range_constexpr(0, inter_dim, BLOCK_THREADS):
                ...
                if col < n:
                    ...
                    if const_expr(act == "swiglu"):
                        ...
                        sigmoid = one / (one + exp2_approx(-(gate * alpha) * log2e))
    ...
    def launch(
        ...
        if not isinstance(num_sorted_rows, int) or num_sorted_rows < 0:
            raise ValueError(f"num_sorted_rows must be a non-negative int, got {num_sorted_rows!r}")
        rows = token_num * topk
        if tuple(x.shape) != (rows, 2 * inter_dim):
            raise ValueError(f"expected x shape ({rows},{2 * inter_dim}), got {tuple(x.shape)}")
        if tuple(out.shape) != (rows, inter_dim):
            raise ValueError(f"expected out shape ({rows},{inter_dim}), got {tuple(out.shape)}")
        ...
        if str(out.dtype) != expected_torch_dtype:
            raise ValueError(f"out dtype must be {expected_torch_dtype}, got {out.dtype}")
        ...
        if sorted_ids.dim() != 1 or int(sorted_ids.numel()) < num_sorted_rows:
            raise ValueError(f"sorted_ids must be 1D with at least {num_sorted_rows} elements")
        ...
        if apply_route_weight:
            ...
            if sorted_weights.dim() != 1 or int(sorted_weights.numel()) < num_sorted_rows:
                raise ValueError("sorted_weights must be 1D with at least " f"{num_sorted_rows} elements")
