...
def compile_iluvatar_mr_moe_gemm1_b16(
    *,
    ...
    def activate(
        ...
        valid = (sorted_row < fx.Int32(num_valid_ids[0])) & (token < fx.Int32(tokens_in)) & (slot < fx.Int32(topk))
        if valid:
            route = fx.Float32(sorted_weights[sorted_row]) if fx.const_expr(apply_route_weight) else fx.Float32(1.0)
            one = fx.Float32(1.0)
            ...
            for base in fx.range_constexpr(0, inter_dim, ACTIVATION_THREADS):
                ...
                if col < fx.Int32(inter_dim):
                    ...
                    up = Workspace[token, slot, col + fx.Int32(inter_dim)].to(fx.Float32)
                    sigmoid = one / (one + exp2_approx(-gate * log2e))
                    Out[token, slot, col] = (gate * sigmoid * up * route).to(elem_dtype)
