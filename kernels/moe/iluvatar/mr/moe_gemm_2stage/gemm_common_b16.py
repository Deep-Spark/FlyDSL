...
def validate_grouped_b16_config(
    *,
    ...
    if warp_atoms_n % 2:
        raise ValueError("warp_atoms_n must be even for the paired MR shuffle epilogue")
...
def build_grouped_b16_kernel(
    *,
    ...
    def grouped_b16_kernel(
        ...
        mma_atom = fx.make_mma_atom(ixdl.MRMma(16, 16, 16, elem_dtype, elem_dtype, fx.Float32))
        ...
        for mma_m in fx.range_constexpr(warp_atoms_m):
            for ei in fx.range_constexpr(4):
                local_m = warp_m_base + fx.Int32(mma_m * ATOM_M + ei * 4) + lane_row
                sorted_row = m_base + local_m
                fused = fx.Int32(sorted_token_ids[sorted_row])
                token = fused & fx.Int32(0xFFFFFF)
                slot = fused.shrui(fx.Int32(24))
                row_valid = (sorted_row < fx.Int32(num_valid_ids[0])) & (token < tokens) & (slot < fx.Int32(topk))
                ...
                for phys_n in fx.range_constexpr(
                    ...
                    value0 = (lane_em == fx.Int32(0)).select(raw0_s0, raw1_s0)
                    value1 = (lane_em == fx.Int32(0)).select(raw0_s1, raw1_s1)
                    global_n0 = n_base + warp_n_base + fx.Int32(phys_n) + lane_col * fx.Int32(2)
                    global_n1 = global_n0 + fx.Int32(1)
                    if row_valid & (global_n0 < fx.Int32(N)):
                        Out[token_safe, slot_safe, global_n0] = (value0 * route_weight).to(elem_dtype)
                    if row_valid & (global_n1 < fx.Int32(N)):
                        Out[token_safe, slot_safe, global_n1] = (value1 * route_weight).to(elem_dtype)
