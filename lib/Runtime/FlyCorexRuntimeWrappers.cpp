//===- FlyCorexRuntimeWrappers.cpp - Iluvatar COREX runtime wrappers ------===//
//
...
//===----------------------------------------------------------------------===//
//
// Thin CUDA Driver API wrappers targeting Iluvatar COREX SDK.
// Exports the mgpu* C ABI expected by MLIR's gpu-to-llvm lowering.
//
//===----------------------------------------------------------------------===//
  ...
  if (rank > 0) {
    ...
    for (int64_t i = rank - 2; i >= 0; --i)
      denseStrides[static_cast<size_t>(i)] =
          sizes[i + 1] * denseStrides[static_cast<size_t>(i + 1)];
  }
  uint64_t sizeBytes = (rank > 0 ? sizes[0] * denseStrides[0] : 1) * elementSizeBytes;
