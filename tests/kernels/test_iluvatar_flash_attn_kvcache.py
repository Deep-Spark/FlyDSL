...
import kernels.attention.iluvatar.flash_attn_kvcache as flash_attn_kvcache_module  # noqa: E402
from kernels.attention.iluvatar.flash_attn_kvcache import flash_attn_with_kvcache  # noqa: E402
...
def test_flash_attn_with_kvcache_v5_hnd_uneven_group(num_splits: int):
    """V5 HND fast path with library-aligned split-group reductions."""
...
def test_flash_attn_with_kvcache_dense_cache_batch_idx_update():
    ...
    cache_seqlens = torch.tensor([3, 4, 6], device=device, dtype=torch.int32)
    ...
    selected_lens = cache_seqlens.index_select(0, cache_batch_idx)
...
def test_flash_attn_with_kvcache_mma_scale_dispatch(monkeypatch, softmax_scale: float | None, expect_mma: bool):
    ...
    assert captured["use_v5_decode"] is False
...
def test_flash_attn_with_kvcache_rejects_invalid_paged_block_table():
    device = "cuda"
    dtype = torch.bfloat16
