...
def _enable_mma_decode(monkeypatch):
    # This module exercises MMA / pipelined-MMA paths. Pin the feature flags so a
    # dirty process env cannot silently fall back to scalar (MMA=0) or steal the
    # shape onto SIMT (SIMT=1 wins planner priority for D128 GQA2).
...
@pytest.mark.parametrize(
    ("upstream_layout", "max_seqlen", "num_splits"),
    [
        (True, 512, 8),  # baseline MMA
        (False, 2048, 16),  # pipelined HND MMA
    ],
)
def test_flash_attn_with_kvcache_mma_decode_ignores_padded_blocks(
    upstream_layout: bool,
    max_seqlen: int,
    num_splits: int,
):
    """Boundary and inactive splits must not read padded block-table slots."""
    torch.manual_seed(419 + max_seqlen)
    device = "cuda"
    dtype = torch.bfloat16
    bsz, seqlen_q, num_heads, num_kv_heads, head_dim = 1, 1, 8, 2, 128
    block_size, active_seqlen = 16, 129
    max_blocks = max_seqlen // block_size
    cache_seqlens = torch.tensor([active_seqlen], device=device, dtype=torch.int32)
    ...
    if upstream_layout:
        k_dense = torch.empty(bsz, max_seqlen, num_kv_heads, head_dim, device=device, dtype=dtype).uniform_(-1, 1)
        v_dense = torch.empty_like(k_dense).uniform_(-1, 1)
        k_cache = _dense_to_paged_upstream(k_dense, block_table, block_size, max_blocks)
        v_cache = _dense_to_paged_upstream(v_dense, block_table, block_size, max_blocks)
        k_ref, v_ref = k_dense, v_dense
    else:
        k_dense = torch.empty(bsz, num_kv_heads, max_seqlen, head_dim, device=device, dtype=dtype).uniform_(-1, 1)
        v_dense = torch.empty_like(k_dense).uniform_(-1, 1)
        ...
        k_ref = k_dense.transpose(1, 2)
        v_ref = v_dense.transpose(1, 2)

    # Keep only blocks intersecting the runtime sequence addressable.  Any
    # capacity-tail prefetch or inactive-split prologue now dereferences -1.
    used_blocks = (active_seqlen + block_size - 1) // block_size
    block_table[:, used_blocks:] = -1
    ref = _attention_ref(
        q,
        k_ref,
        v_ref,
    ...
    out = flash_attn_with_kvcache(
        ...
        block_table=block_table,
        causal=True,
        force_upstream_cache_layout=upstream_layout,
        num_splits=num_splits,
    )
    torch.cuda.synchronize()
    torch.testing.assert_close(out.float(), ref.float(), atol=3e-2, rtol=3e-2)
