...
def _sort_routes(topk_ids, topk_weights, *, experts: int, block_size: int):
    ...
    sorted_ids = torch.full(
        (max_rows,), sentinel, dtype=torch.int32, device=topk_ids.device
    )
    sorted_weights = torch.zeros(
        (max_rows,), dtype=torch.float32, device=topk_ids.device
    )
    expert_ids = torch.full(
        (max_blocks,), -1, dtype=torch.int32, device=topk_ids.device
    )
    row_begin = 0
    block_begin = 0
    for expert in range(experts):
        ...
        if count:
            sorted_ids[row_begin : row_begin + count] = (
                slot.to(torch.int32) << 24
            ) | token.to(torch.int32)
            sorted_weights[row_begin : row_begin + count] = topk_weights[
                token, slot
            ]
        if blocks:
    ...
    num_valid_ids = torch.tensor(
        [row_begin, tokens], dtype=torch.int32, device=topk_ids.device
    )
    return (
...
def _reference_stage1(x, w1, topk_ids, topk_weights, apply_route_weight):
    ...
    out = torch.empty(
        (tokens, topk, inter_dim), dtype=torch.float32, device=x.device
    )
...
def _reference_stage2(x, w2, topk_ids, topk_weights, apply_route_weight):
    ...
    per_slot = torch.empty(
        (tokens, topk, model_dim), dtype=torch.float32, device=x.device
    )
    for token in range(tokens):
        for slot in range(topk):
            expert = int(topk_ids[token, slot])
...
def test_gemm1_compile_guards(monkeypatch, kwargs, match):
    ...
    options = dict(
        model_dim=64, inter_dim=64, experts=4, topk=2, dtype="bf16"
    )
...
def test_gemm2_rejects_unreliable_b16_atomics(monkeypatch):
    ...
    with pytest.raises(ValueError, match="ATOMIC"):
        compile_iluvatar_mr_moe_gemm2_b16_ex(
            mode=MoeGemm2Mode.ATOMIC, **options
        )
...
def test_gemm1_correctness(monkeypatch, dtype, apply_route_weight):
    ...
    w1 = (
        torch.randn(experts, 2 * inter_dim, model_dim, device="cuda") * 0.05
    ).to(td)
    ...
    sorted_ids, sorted_weights, expert_ids, num_valid_ids, num_blocks = (
        _sort_routes(
            topk_ids,
            topk_weights,
            experts=experts,
    ...
    workspace = torch.zeros(
        tokens, topk, 2 * inter_dim, device="cuda", dtype=td
    )
...
def test_gemm2_per_slot_and_reduction(monkeypatch, dtype):
    ...
    w2 = (
        torch.randn(experts, model_dim, inter_dim, device="cuda") * 0.05
    ).to(td)
    topk_ids = torch.stack(
        [torch.randperm(experts, device="cuda")[:topk] for _ in range(tokens)]
    ).to(torch.int32)
    ...
    per_slot = torch.zeros(
        tokens, topk, model_dim, device="cuda", dtype=td
    )
    ...
    torch.testing.assert_close(
        out.float(), ref_slot.sum(dim=1), rtol=tol, atol=tol
    )
