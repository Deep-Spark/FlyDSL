...
def _make_sorted_ids(token_num: int, topk: int, padding: int):
    packed = [(slot << 24) | token for token in range(token_num) for slot in range(topk)]
...
def test_iluvatar_silu_and_mul_correctness(
    ...
    rows = token_num * topk
    x_separated = ((torch.rand(rows, 2 * inter_dim, device="cuda", dtype=torch.float32) * 6.0) - 3.0).to(torch.bfloat16)
    ...
    if enable_bias:
        ...
        bias = (torch.rand(experts, 2 * inter_dim, device="cuda", dtype=torch.float32) - 0.5).contiguous()
    else:
        topk_ids = None
        bias = None
...
def test_iluvatar_silu_and_mul_dtype_and_route_weight(monkeypatch, dtype, torch_dtype):
    _configure_iluvatar_env(monkeypatch)
    token_num, topk, inter_dim = 3, 2, 256
    rows = token_num * topk
    torch.manual_seed(123)
    x = (torch.randn(rows, 2 * inter_dim, device="cuda", dtype=torch.float32) * 0.5).to(torch_dtype)
    sorted_ids = _make_sorted_ids(token_num, topk, 0)
    route_by_row = torch.rand(rows, device="cuda", dtype=torch.float32)
    sorted_weights = torch.empty(rows, device="cuda", dtype=torch.float32)
    ...
    expected = torch.nn.functional.silu(gate) * up * route_by_row[:, None]
