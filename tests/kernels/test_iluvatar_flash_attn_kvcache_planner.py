...
def test_flash_attn_with_kvcache_uses_max_context_len_for_pipelined_split_planning(monkeypatch, num_kv_heads: int):
    ...
    # SIMT compiles against capacity because split extents are runtime-driven.
    # Pipelined MMA bakes split tile ranges in and uses the active power-of-two
    # planning bucket.
    assert captured["max_seqlen_k"] == (512 if num_kv_heads == 2 else 2048)
...
def test_cached_simt_launch_returns_independently_owned_outputs(monkeypatch):
    ...
    def fake_compile(_launcher, *compile_args):
        fake_compiled(
            *(getattr(arg, "torch_tensor", arg) for arg in compile_args)
        )
        return fake_compiled

    monkeypatch.setattr(flash_attn_kvcache_module.flyc, "compile", fake_compile)
    q = torch.empty(1, 1, 16, 128, device="cuda", dtype=torch.bfloat16)
