...
def test_iluvatar_pipeline_not_wired_yet(monkeypatch):
    backends = _load_backends(monkeypatch)
    backend = backends.get_backend("iluvatar", arch="ivcore11")

    with pytest.raises(NotImplementedError, match="pipeline is not wired"):
        backend.pipeline_fragments(compile_hints={})
