...
// Minimal IXDL MMA atom lowering test:
//   fly.mma_atom_call -> ixdl.mmad

// CHECK-LABEL: @test_ixdl_mma_atom_call
// CHECK-SAME: (%[[D:.*]]: !llvm.ptr<5>, %[[A:.*]]: !llvm.ptr<5>, %[[B:.*]]: !llvm.ptr<5>, %[[C:.*]]: !llvm.ptr<5>)
func.func @test_ixdl_mma_atom_call(
  ...
  fly.mma_atom_call(%atom, %d, %a, %b, %c) : (!fly.ixdl_mmad<16x16x16, (f16, f16) -> f32>, !fly.memref<f32, register, 4:1>, !fly.memref<f16, register, 4:1>, !fly.memref<f16, register, 4:1>, !fly.memref<f32, register, 4:1>) -> ()
  return
}
