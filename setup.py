...
class build_py(_build_py):
    """Copy sources, optionally baking DUMP_SUPPORT=False into release wheels."""

    def copy_file(self, infile, outfile, preserve_mode=1, preserve_times=1, link=None, level=1):
        src = Path(infile)
        if src.name == "dump_support.py" and os.environ.get("FLYDSL_RELEASE_STRIP_DUMP", "").strip().lower() in {
            "1",
            "true",
            "yes",
            "on",
            ...
            dest.write_text(
                "# SPDX-License-Identifier: Apache-2.0\n"
                "# Generated by setup.py for FLYDSL_RELEASE_STRIP_DUMP=1\n"
                "DUMP_SUPPORT = False\n"
                "\n"
                "\n"
...
setup(
    ...
    cmdclass=(
        {"build_py": build_py, "bdist_wheel": bdist_wheel}
        if bdist_wheel is not None
        else {"build_py": build_py}
    ),
