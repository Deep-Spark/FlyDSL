...
kernels/attention/iluvatar/
  ...
  flex_attn_interface.py  # flydsl_flex_attn_func (Torch dispatcher)
```
...
| **Flex-attention** | V2 forward (dense / varlen / paged) via `compile_*` + `flydsl_flex_attn_func` | No user `score_mod`, no backward. Relative to Torch SDPA still has a gap on long sequences. |
