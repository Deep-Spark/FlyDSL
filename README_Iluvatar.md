...
| **Production HGEMM** | `kernels.gemm.iluvatar.mr.hgemm` — double-buffered G2S, Ki-deferred S2R/MMA, configurable epilogue and major pattern |
...
Measured on **Iluvatar BI-V150S** (`ARCH=ivcore11`), using `kernels.gemm.iluvatar.mr.hgemm`
...
| **B8 / INT8 GEMM** | Not implemented | Low-level pieces exist (`MRAsyncCpRow8b`, `MRMma` i8 path in FlyIXDL), but there is no pipelined **INT8/B8 GEMM** kernel module comparable to `kernels.gemm.iluvatar.mr.hgemm`. |
| **Other production kernels** | ROCm-only today | The main FlyDSL `kernels/` portfolio (FP8/INT4 preshuffle GEMM, blockscale GEMM, MoE, paged/flash attention, LayerNorm/RMSNorm/Softmax, fused RoPE, all-reduce, quant, etc.) has **no Iluvatar port** yet. Only **f16 HGEMM** (`kernels.gemm.iluvatar.mr.hgemm`) and teaching/unit-test coverage are in tree. |
