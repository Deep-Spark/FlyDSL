...
| **Pipeline sync** | **MR:** `sl_waitmem` + `sl_barrier_alu` + `sl_pipebar_arrive` / `sl_pipebar_wait` (`pipe-bar`). **CQ:** `nbarrier_reach` / `nbarrier_wait` / `nbarrier_sync` (`named-bar` / `named-bar-sync`). Do not emit pipebar on CQ. See `python/flydsl/expr/ixdl/sync.py` |
