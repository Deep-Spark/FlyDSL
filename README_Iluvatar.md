...
| **Flex-attention** | V2 forward (dense / varlen / paged) via `compile_*` + `flydsl_flex_attn_func`; optional `tile_config` / dense `autotune_*_tile`; dense `score_mod=TracedScoreMod` (V3-2; dispatcher rejects); dense `return_lse` + `compile_*_bwd` (MHA bring-up) | Still a variant subset (not a general score_mod compiler / no BlockMask yet). Bwd is dense-MHA correctness-first (not MMA-opt). Relative to Torch SDPA still has a gap on long sequences. |
