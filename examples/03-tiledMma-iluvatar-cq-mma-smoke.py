...
"""CQ CQMma fragment smoke (not a pipelined HGEMM/IGEMM).

Single-warp CQMma with constant A/B fragments. Hand-run on ivcore30 to validate
base vs FeatureLongMtx tiles. Requires a CQ device; not part of default CI.

Examples::

    ARCH=ivcore30 FLYDSL_COMPILE_BACKEND=iluvatar \\
      python examples/03-tiledMma-iluvatar-cq-mma-smoke.py --check

    ARCH=ivcore30 FLYDSL_COMPILE_BACKEND=iluvatar \\
      python examples/03-tiledMma-iluvatar-cq-mma-smoke.py --check --mma-tile 32x32

    ARCH=ivcore30 FLYDSL_COMPILE_BACKEND=iluvatar \\
      python examples/03-tiledMma-iluvatar-cq-mma-smoke.py --check --dtype s8 --mma-tile 16x64
"""
...
def _run_check(*, dtype: str, mma_tile: str) -> None:
    ...
    from kernels.gemm.iluvatar.cq.mma_fragment_smoke import (
    ...
    if dtype == "s8":
        launch = compile_cq_mma_fragment_smoke_s8(mma_tile=mma_tile, a_value=1, b_value=2)
        ...
        launch = compile_cq_mma_fragment_smoke_f16(
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--check", action="store_true", help="run CQMma fragment correctness check")
    p.add_argument(
        "--mma-tile",
        default="base",
