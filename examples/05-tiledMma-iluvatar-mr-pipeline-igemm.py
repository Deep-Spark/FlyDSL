...
_HGEMM_EXAMPLE = "examples/03-tiledMma-iluvatar-mr-pipeline-hgemm.py"

_DOC = """Iluvatar MR (ivcore11) tiledMma pipeline IGEMM: int8 inputs, int32 output.
...
* 16x16x32 ``MRMma`` atom (int8 A/B, int32 accumulator) -- vs f16's 16x16x16.
...
Run::
    ...
    python examples/05-tiledMma-iluvatar-mr-pipeline-igemm.py --check
    python examples/05-tiledMma-iluvatar-mr-pipeline-igemm.py --bench
"""
__doc__ = _DOC.format(hgemm_example=_HGEMM_EXAMPLE)
...
from kernels.iluvatar_mr_common import (  # noqa: E402
    DEFAULT_MAJOR_PATTERN,
    MAJOR_PATTERN_CHOICES,
    WARP_SIZE,
)
from kernels.iluvatar_mr_igemm import (  # noqa: E402
    ATOM_K_I8,
    DEFAULT_EPILOGUE,
    DEFAULT_K_REP,
    EPILOGUE_CHOICES,
...
def _remap_for_pattern(A, B, major_pattern: str):
...
def _check(
    ...
    a_dev, b_dev = _remap_for_pattern(A, B, major_pattern)
...
def _parse_args(argv):
    ...
    p.add_argument(
        ...
        help="G2S global layout tag for A/B (see kernels.iluvatar_mr_igemm)",
    )
    p.add_argument(
        "--epilogue",
...
def main(argv=None):
    ...
    if compile_only or not torch.cuda.is_available():
        ...
        a_dev, b_dev = _remap_for_pattern(a, b, args.major_pattern)
