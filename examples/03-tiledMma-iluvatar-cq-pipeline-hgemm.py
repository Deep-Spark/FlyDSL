...
Usage::
    ...
    # --cta auto (default) picks 256 / 1024 / 2048 by shape independently for
    # check vs bench.
    python examples/03-tiledMma-iluvatar-cq-pipeline-hgemm.py --bench \\
      --dtype fp16 --m 4096 --n 4096 --k 4096 \\
...
# Reserved CQ GPU for this workspace -- do not default onto other devices.
os.environ.setdefault("CUDA_VISIBLE_DEVICES", "15")
...
def _resolve_cta(
    args: argparse.Namespace, m: int, n: int, k: int
) -> tuple[SwizzleCtaPreset, dict[str, int]]:
    """Resolve CTA preset + launch kwargs for ``(m, n, k)``.

    ``--cta auto`` picks via ``select_swizzle_cta``; an explicit preset name
    forces that config. CLI overrides (``--warps-m`` / ``--k-atoms`` / ...) win
    over preset defaults for any field that is set.
    """
    if args.cta == SWIZZLE_CTA_AUTO:
        preset = select_swizzle_cta(m, n, k)
    else:
        preset = SWIZZLE_CTA_PRESETS[args.cta]
    kwargs = {
        "warps_m": preset.warps_m if args.warps_m is None else args.warps_m,
        "warps_n": preset.warps_n if args.warps_n is None else args.warps_n,
        "warp_atoms_m": preset.warp_atoms_m if args.warp_atoms_m is None else args.warp_atoms_m,
        "warp_atoms_n": preset.warp_atoms_n if args.warp_atoms_n is None else args.warp_atoms_n,
        "k_atoms": preset.default_k_atoms if args.k_atoms is None else args.k_atoms,
    }
    return preset, kwargs


def _log_cta(
    preset: SwizzleCtaPreset, m: int, n: int, k: int, cta: dict[str, int], *, auto: bool
) -> None:
    bm, bn, bk, threads, smem = _swizzle_cta_shape(
        cta["warps_m"],
        cta["warps_n"],
        cta["k_atoms"],
        warp_atoms_m=cta["warp_atoms_m"],
        warp_atoms_n=cta["warp_atoms_n"],
    )
    tag = "auto ->" if auto else "cta ="
    print(
        f"[cta] {tag} {preset.name} for M={m} N={n} K={k} "
        f"(tile {bm}x{bn}x{bk}, threads={threads}, smem={smem})"
    )


def _validate_shape(
    m: int, n: int, k: int, cta: dict[str, int], *, major_pattern: str
) -> str | None:
    ...
    if k_atoms < 2 or k_atoms % 2:
        return f"k_atoms must be a positive even integer, got {k_atoms}"
    if k % bk:
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "  python examples/03-tiledMma-iluvatar-cq-pipeline-hgemm.py --bench \\\n"
            "    --m 4096 --n 4096 --k 4096 --warmup 5 --iters 50\n"
            "  python examples/03-tiledMma-iluvatar-cq-pipeline-hgemm.py --bench \\\n"
            "    --m 1024 --n 1024 --k 1024 --cta 1024 --k-atoms 2 --warmup 5 --iters 50\n"
        ),
    )
    ...
    p.add_argument(
        "--cta",
        choices=[SWIZZLE_CTA_AUTO, *sorted(SWIZZLE_CTA_PRESETS)],
        default=SWIZZLE_CTA_AUTO,
        help=(
            "CTA preset: auto (default; pick by M/N/K), "
            "256 (64x64), 1024 (256x256), 2048 (512x256, 2048 threads, ka4)"
        ),
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=None,
        help="BK = 16 * k_atoms (even); default from --cta preset",
    )
    p.add_argument(
        "--check-shape",
        nargs=3,
    ...
    auto = args.cta == SWIZZLE_CTA_AUTO

    # Always run a correctness check first (MR harness behavior).
    # With --cta auto, check and bench may pick different presets by shape.
    cm, cn, ck = args.check_shape
    try:
        check_preset, check_cta = _resolve_cta(args, cm, cn, ck)
    except ValueError as exc:
        print(f"Error (--check-shape): {exc}", file=sys.stderr)
        return 2
    _log_cta(check_preset, cm, cn, ck, check_cta, auto=auto)
    err = _validate_shape(cm, cn, ck, check_cta, major_pattern=args.major_pattern)
    if err:
    ...
    try:
        bench_preset, bench_cta = _resolve_cta(args, args.m, args.n, args.k)
    except ValueError as exc:
        print(f"Error (--m/--n/--k): {exc}", file=sys.stderr)
        return 2
    if auto or bench_preset.name != check_preset.name:
        _log_cta(bench_preset, args.m, args.n, args.k, bench_cta, auto=auto)
    err = _validate_shape(args.m, args.n, args.k, bench_cta, major_pattern=args.major_pattern)
    if err:
