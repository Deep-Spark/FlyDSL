...
"""Correctness and performance driver for MR BF16 implicit-GEMM Conv3D.

Run from the FlyDSL repository root:

    python examples/06-conv3d-iluvatar-mr-no-sme-f.py --check
    python examples/06-conv3d-iluvatar-mr-no-sme-f.py --bench

The full command like:
    python examples/06-conv3d-iluvatar-mr-no-sme-f.py --check --n 1 --c 64 --d 16 --h 32 --w 32 \
                    --k 128 --kt 3 --kh 3 --kw 3 --groups 1 --stride 1 --padding 1 --dilation 1 \
                    --scale 0.25 --rtol 0.02 --atol 0.02 --seed 2026
...
from kernels.conv.iluvatar.mr.conv3d_implicit_no_sme_bf16 import (
    BLOCK_K,
    BLOCK_M,
...
def _build(args, x, weight):
    ...
    smem = (BLOCK_M + meta["block_n"]) * BLOCK_K * 2
    return launch, x_packed, weight_packed, y_workspace, meta, grid, block, smem
...
def _check(args):
    ...
    actual = unpack_conv3d_implicit_no_sme_output(yw, meta)
    expected = _reference(args, x, weight)
    diff = (actual.float() - expected.float()).abs()
    ok = torch.allclose(actual.float(), expected.float(), rtol=args.rtol, atol=args.atol)
    print(
        ...
        f"max_abs={float(diff.max()):.6g} mean_abs={float(diff.mean()):.6g}"
    )
    if not ok:
        mismatch = (diff > args.atol + args.rtol * expected.float().abs()).nonzero()
        first = tuple(int(v) for v in mismatch[0])
        print(f"  first mismatch at {first}: got={float(actual[first])}, expected={float(expected[first])}")
    return ok
...
def _bench(args):
    ...
    try:
        ...
        print(f"  [info] torch BF16 Conv3D baseline unavailable: {repr(exc)[:120]}")

    baseline = f" (torch-bf16 {torch_us:.1f} us)" if torch_us is not None else ""
    print(
        f"[bench] x={tuple(x.shape)} w={tuple(weight.shape)} groups={args.groups} "
    ...
    if not torch.allclose(actual.float(), expected.float(), rtol=args.rtol, atol=args.atol):
        diff = (actual.float() - expected.float()).abs()
        print(f"  [WARN] post-bench correctness FAILED (max_abs={float(diff.max()):.6g})")
        return False
    return True


def _parse_args(argv):
    p = argparse.ArgumentParser(description="Iluvatar MR BF16 implicit-GEMM Conv3D NoSme")
...
def main(argv=None):
    ...
    if compile_only or not torch.cuda.is_available():
        ...
        launch(xp, wp, yw)
        print(
            f"Compiled MR BF16 implicit Conv3D NoSme (COMPILE_ONLY; "
            f"grid={grid}, block={block}, smem={smem}, "
    ...
    # Like the GEMM examples, a no-flag invocation performs a small correctness
    # check before users move to the larger benchmark defaults.
